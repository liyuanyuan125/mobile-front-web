<template>
 <div class="event-content">
    <div class="title">
      {{eventName}}
      <router-link class="to-more" to="" ><van-icon name="arrow" size="20" /></router-link>
    </div>
    <ul>
      <li v-for="item in List" :key="item.eventId" class="flex-box flex-between">
        <div>
          ...
          <p class="flex-box">
            <span v-for="it in item.interactiveList" class="counts flex-box">
...
export default class Main extends Vue {
  /** 事件title */
  @Prop({ type: String, default: '事件分析'}) eventName!: string
  /** 更多事件分析 跳转参数 */
  ...
  get List() {
    const list = this.eventList.map((it: any) => {
      ...
      const creatTime = time1 < 11 ? `${time1}天前` : moment(1583154943041).format('YYYY-MM-DD')
